...
public class MatrixUtils {
    ...
    public static double[][] accumulateVertical(double[][] m) {
        ...
        for (int x = 0; x < m.length - 1; x += 1) {
            n[x + 1][0] = m[x + 1][0] + n[x][0];
            if (n[x][1] < n[x][0]) {
                n[x + 1][0] = m[x + 1][0] + n[x][1];
            }
            for (int y = 1; y < m[0].length - 1; y += 1) {
                if (n[x][y - 1] <= n[x][y] && n[x][y - 1] <= n[x][y + 1]) {
                    n[x + 1][y] = m[x + 1][y] + n[x][y - 1];
                } else if (m[x][y] <= m[x][y + 1]) {
                    n[x + 1][y] = m[x + 1][y] + n[x][y];
                } else {
                    n[x + 1][y] = m[x + 1][y] + n[x][y + 1];
                }
            }
            int last = m[0].length - 1;
            n[x + 1][last] = m[x + 1][last] + n[x][last];
            if (n[x][last - 1] < n[x][last]) {
                n[x + 1][last] = m[x + 1][last] + n[x][last - 1];
            }
        }
        return n;
    }

    /** Returns the Transpose Matrix of M with the given ORIENTATION.
     */
    ...
    public static double[][] accumulate(double[][] m, Orientation orientation) {
        if (orientation == Orientation.VERTICAL) {
            ...
            throw new IllegalArgumentException("input correct orientation");
        }
    }
    ...
    public static int[] findVerticalSeam(double[][] m) {
        ...
        int w = m[0].length;
        int[] seam = new int[h];
        seam[h - 1] = findMinIndex(m[h - 1], 0, w - 1);
        for (int x = h - 2; x >= 0; x -= 1) {
            seam[x] = findMinIndex(m[x], seam[x + 1] - 1, seam[x + 1] + 1);
        }
        return seam;
    }

    /** An helper function that takes  that takes
     *  an array AR, a LO index, and a HI index, and returns the
     *  index of the smallest thing in that array between those
     *  indices (inclusive).*/
    public static int findMinIndex(double[] ar, int lo, int hi) {
        lo = Math.max(lo, 0);
        hi = Math.min(hi, ar.length - 1);
        int index = lo;
        double min = ar[lo];
